...
{
    [Theory]
    [InlineData(AiProvider.Anthropic, "claude-sonnet-4-6")]
    [InlineData(AiProvider.Anthropic, "claude-haiku-4-5-20251001")]
    [InlineData(AiProvider.OpenAi, "gpt-4.1")]
    [InlineData(AiProvider.OpenAi, "gpt-4.1-mini")]
    [InlineData(AiProvider.Xai, "grok-4.3")]
    ...
    [InlineData(AiProvider.Xai, "claude-sonnet-4-6")]   // real model, wrong provider — must not match
    ...
    {
        Assert.True(LlmPricingCatalog.TryGetRate(AiProvider.Anthropic, "claude-sonnet-4-6", out var rates));
