...
- **All server calls are TanStack Query** mutations/queries in `hooks/` (`useCreateSession`, `useStartChallenge`, `useSubmitAttempt`, etc.). No raw `useEffect` + `fetch`. `QueryClient` is configured in `App.tsx` (`retry: 1`, `refetchOnWindowFocus: false`).
- **Streaming chat is one shared deep Module:** `useStreamingChat` (`src/hooks/`) owns the whole client-side turn invariant for all three surfaces — transcript state (`ChatMessage[]`), optimistic user append, delta accumulation into `streamingText` (via `useStreamingText`), assistant append on success, and the failure mirror of the server's whole-turn rollback (drop the optimistic bubble, snapshot the partial reply into `failedTurn`, restore the message as an input `draft`). A surface supplies one `streamTurn` function as data (its `stream*` apiClient call, closing over current editor/justification state); per-send caller intent rides a generic `context` (e.g. the tutoring surface's `GuidanceMode`) and surface-specific reply handling rides a per-send `onSuccess` (context-token capture). The shared `ChatTranscript` component (`features/chat/components/`) renders the transcript: bubbles, the `StreamingChatTail` (live reply / dimmed failed turn), the empty-state hint, and keep-scrolled-to-bottom; `ChatInput`'s `draft` prop completes the failed-turn restore. The former per-surface chat hooks (`useSendMessage`, `usePromptLabChat`, `useSystemLabChat`) and the per-lab chat message types are deleted — all surfaces use the one `ChatMessage`.
- The three surfaces still keep thin per-surface start/submit/get hooks (`useStartChallenge`, `useSubmitAttempt`, …) — deliberate pass-throughs; consolidating them earns little.
...
**Stream Envelope** — the Api-side Module (`NdjsonStreamEnvelope`) every `/stream` endpoint runs inside. It owns the status-line-freeze invariant of the Chunk contract: pre-stream failures propagate for real HTTP statuses, post-start failures ride the stream as error events, caller cancellation ends the stream silently, and the final event is written from the stream body's return value. Endpoints supply only their service call.
**Streaming Chat** — the client-side Module owning one surface's chat transcript and turn invariant: `useStreamingChat` (state + optimistic append + rollback mirror of the server's whole-turn rollback) and `ChatTranscript` (bubbles + live tail + scroll behavior). Each surface is an Adapter supplying one `streamTurn` function as data.
