...
/// SecurityPitfalls must never be sent to the client — they drive evaluator-only negative scoring.
/// </summary>
public class Scenario
{
    public string ScenarioId { get; set; } = string.Empty;                 // Stable slug, e.g. "identity-rbac-easy-01"
    ...
    public List<RubricCriterion> Rubric { get; set; } = [];                 // Scoring criteria; security criteria use "security-" prefix
    public List<string> RequiredTradeoffs { get; set; } = [];               // Visible to user; authored as causal questions
    public List<string> SecurityPitfalls { get; set; } = [];                // NEVER expose to client — evaluator checks these for deductions
    public int MaxSecurityDeduction { get; set; }                           // Maximum points that can be deducted for security failures
}
